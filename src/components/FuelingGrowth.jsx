...
const stats = [
  { label: "Cost Reduction", percent: 25 },
  { label: "Productivity Boost", percent: 100 },
  { label: "Client Satisfaction", percent: 100 },
];

const RADIUS = 38; // Increased from 28 to 38
...
function CircleProgress({ percent, animate }) {
  ...
  return (
    <div className="relative w-[96px] h-[96px]"> {/* Increased from 72px to 96px */}
      <svg width="96" height="96" viewBox="0 0 96 96" className="-rotate-90" aria-hidden="true">
        <circle cx="48" cy="48" r={RADIUS} fill="none" stroke="#e8ecf5" strokeWidth="6" />
        <circle
      ...
      <span className="absolute inset-0 flex items-center justify-center text-2xl font-bold text-[#1a2f6e]">
        {percent}%
      </span>
    </div>
  );
        ...
        <div className="sm:max-w-7xl sm:mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.4fr] gap-10 md:gap-16 items-center">
            ...
            <div className="text-center lg:text-left">
              ...
              <button className="bg-blue-600 hover:bg-blue-700 active:scale-95 text-white text-sm sm:text-xl px-6 py-3 rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl">
                Get a Free Consultation
              </button>
            </div>
